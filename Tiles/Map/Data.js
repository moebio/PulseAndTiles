receiveData = function(dataObj){
  ...
  switch(dataObj.type){
    ...
    case "center":
      setCenter(dataObj.value)
      break

    case "over":
      setOver(dataObj.value)
      break
    ...
    case "configuration": //not working yet
      setConfiguration(dataObj.value)
      break

    case "tile":
      setLayer(dataObj.value)
      break
...
setOver = function(index){
  if(index<0) return
  externalOverObject = {point:geoPoints[index], index}
}

setCenter = function(centerObject){
  goToCoordinates(centerObject.lat, centerObject.long, centerObject.zoom, centerObject.duration)
}


setColors = function(array){
  this.colors = array==null?null:(array?.type=="cL"||_.areColor(array)?array:array.toL().toColorList())
}

setWeights = function(array){
  array.forEach((weight,i)=>{
    geoPoints[i].finalWeight = weight
    //geoPoints[i].weight = weight
  })
}
...
setCoordinates = function(table){
  ...
  lat.forEach((lt,i)=>{
    ...
    const gPoint = new _.P(long[i] + (Math.random()-0.5)*0.01, lt + (Math.random()-0.5)*0.01)
    gPoint.weight = gPoint.finalWeight = 1
    geoPoints.push(gPoint)
  })
