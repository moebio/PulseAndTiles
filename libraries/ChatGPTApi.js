...
let urlCompletions = 'https://api.openai.com/v1/chat/completions'
...
let prompt_stack = true //used to stack prompt requests (FIFO), with delay_in_completion delay between them
//////
...
let completion = function (query, onLoad, type, system_message, temperature = 0.5, onError, question, response_format) {
	...
	console.log("[CHGPT] completion called")

	//tmporary
	if (CHAT_GPT_API_MODEL.includes("o1-")) temperature = 1
	...
	const timeSinceLastCompletionQuery = lastCompletionActivityDate ? (new Date().getTime() - lastCompletionActivityDate.getTime()) : 99999
	if (delay_in_completion > 0 && timeSinceLastCompletionQuery < delay_in_completion) {
	...
	_completionRequest(query, onLoad, onError, temperature, system_message, undefined, response_format)
}


let _completionRequest = function (query, onLoad, onError, temperature, system_message, model, response_format) {

	let messages = [{ role: "user", content: query }]
	if (system_message) messages = [{ role: "system", content: system_message }, { role: "user", content: query }]

	if (model) CHAT_GPT_API_MODEL = model

	if (WAITING_RESPONSE_complete) {
		PROMPTS_STACK.push({ query, onLoad, onError, temperature, system_message, model: CHAT_GPT_API_MODEL, response_format })
		console.log("[CHGPT] busy right now, stacking the prompt, PROMPTS_STACK.length:", PROMPTS_STACK.length)
		return
	}
	...
	console.log("[CHGPT] not busy, go!", (new Date()).getHours() + ":" + (new Date()).getMinutes() + ":" + (new Date()).getSeconds())
	...
	if (CHAT_GPT_API_KEY) headers.Authorization = "Bearer " + CHAT_GPT_API_KEY

	const model_used = CHAT_GPT_API_MODEL
	const date = new Date()
	...
	console.log("[ChatGPTApi] CHAT_GPT_API_MODEL:", CHAT_GPT_API_MODEL)
	console.log("[ChatGPTApi] completion -> fetch, messages:", messages)
	...
	fetch(urlCompletions, {
		...
		.then(data => {
			...
			lastCompletionActivityDate = new Date()
			_completionAnswerProcess(data, query, onLoad, onError, model_used, date)
		}).catch(error => {
...
let _completionAnswerProcess = function (data, query, onLoad, onError, model, dateQuery) {
	...
	if (incomplete) {
		...
		if (build_LLM_Activities_Report) {
			console.log("[CHGPT] model for LLM_ACTIVITIES_REPORT:", model)

			timeSinceLastResponse = date_last_llm_response ? Math.round(((new Date()).getTime() - (date_last_llm_response.getTime())) / 1000) : null
			const elapsed_time = Math.round(((new Date()).getTime() - dateQuery.getTime()) / 1000)

			LLM_ACTIVITIES_REPORT.push({
				model,
				time_query: _.dateToString(dateQuery, 10),
	...
	if (prompt_stack && PROMPTS_STACK.length > 0) {
		...
		console.log("[CHGPT] more on the stack! PROMPTS_STACK.length", PROMPTS_STACK.length + " | wait")
		console.log("**** nextPrompt", nextPrompt)
		setTimeout(t => {
			console.log("[CHGPT] --> _completionRequest")
